...
void estimator_velocity_update_dt(estimator_velocity_t *est, float measured_pos, float dt) {
    ...
    mat3x3_mul(state_covariance_pred, covariance_pred, state_transition_transp); // F @ P @ F.T
    ...
    vec3_add(est->x, est->x, offset_v);

    float KH = vec3_dot_product(K, H);
    mat3x3_add_scalar(est->state, est->state, KH);
